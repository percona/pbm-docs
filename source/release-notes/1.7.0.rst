...
* Support for :ref:`physical backups <backup-types>` in |PSMDB| starting from versions 4.2.15-16 and 4.4.6-8 and higher. Physical backups drastically speed up backup and restore performance for large databases (several terabytes). This is a technical preview feature. 
...
To use physical backups with filesystem-based backup storage, the `mongod` user must have the write access to the backup directory. 
