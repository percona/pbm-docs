...
=== "🛡️ Workload Identity Federation"
    ...
    With WIF, you can upload backups to GCS without using long-lived service account JSON keys. PBM authenticates using short-lived tokens obtained through federation with an external identity provider (IdP).
...
=== "🔐 Selective backups"
    ### Selective backups with users and roles
