...
- [PBM-1721](https://perconadev.atlassian.net/browse/PBM-1721): Improved physical restores behavior when a restore would hang indefinitely if PBM encountered an "Unauthorized" error while attempting to shut down a `mongod` node from a remote connection. The restore process now correctly detects this error and terminates with a descriptive message, clarifying that localhost connections are required for node shutdown when running MongoDB without authentication. This use case was added to [Known limitations for backups and restores](../features/known-limitation.md).
