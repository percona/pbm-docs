...
- [PBM-1600](https://perconadev.atlassian.net/browse/PBM-1600): We have added a confirmation prompt to the `pbm restore`, `pbm cleanup`, `pbm delete-pitr`, and `pbm delete` commands to help avoid accidental restores. Before the restore starts, you will now have to explicitly confirm the action. This change is intended to reduce the risk of unintentionally running a command that cannot be undone.
...
- [PBM-1721](https://perconadev.atlassian.net/browse/PBM-1721): Improved physical restores behavior when a restore would hang indefinitely if PBM encountered an "Unauthorized" error while attempting to shut down a `mongod` node via a non-localhost connection interface. The restore process now correctly detects this error and terminates with a descriptive message, clarifying that localhost connections are required for node shutdown when running MongoDB without authentication. This use case was added to [Known limitations for backups and restores](../features/known-limitation.md).
