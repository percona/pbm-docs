...
To enable S3 debug logging, set the `storage.minio.DebugLogLevel` option in Percona Backup for MongoDB configuration. The supported values are: `LogDebug`, `Signing`, `HTTPBody`, `RequestRetries`, `RequestErrors`, `EventStreamBody`.
...
When the option is undefined, the MinIO Standard (`STANDARD`) storage class is used.
